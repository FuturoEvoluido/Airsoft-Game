...
import { AlertTriangle, BadgeCheck, ChevronLeft, Crosshair, Crown, ExternalLink, FileText, Footprints, HardDrive, Image as ImageIcon, Medal, Radio, RotateCcw, Shield, Target, TimerReset, Trophy, UploadCloud, X, Zap } from "lucide-react";
...
function StorageVaultModal({ onClose }: { onClose: () => void }) {
  const { isAuthenticated, loading: authLoading } = useAuth();
  const filesQuery = trpc.files.list.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  const uploadMutation = trpc.files.upload.useMutation({ onSuccess: () => filesQuery.refetch() });
  const [message, setMessage] = useState("");

  const upload = async (file: File) => {
    if (file.size > 10_000_000) { setMessage("Limite operacional: 10 MB por arquivo."); return; }
    setMessage("Codificando arquivo para o storage seguro…");
    try {
      const data = await readFileAsBase64(file);
      await uploadMutation.mutateAsync({ fileName: file.name, mimeType: file.type || "application/octet-stream", size: file.size, data });
      setMessage("Arquivo sincronizado no cofre do operador.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Falha no upload. Tente novamente.");
    }
  };

  return <div className="storage-overlay"><section className="storage-modal"><header><div><p className="eyebrow">BACKEND / S3 SEGURO</p><h2>COFRE <span>TÁTICO</span></h2><p className="storage-subtitle">Briefings, fotos de treino e documentos ficam vinculados ao seu usuário.</p></div><button type="button" className="modal-close" onClick={onClose} aria-label="Fechar"><X size={16} /></button></header>{authLoading ? <div className="storage-state"><HardDrive size={22} /><p>Verificando credenciais do operador…</p></div> : !isAuthenticated ? <div className="storage-state"><HardDrive size={24} /><h3>LOGIN NECESSÁRIO</h3><p>Entre com sua conta Manus para ativar o armazenamento persistente e privado.</p><button type="button" className="storage-login" onClick={() => startLogin()}>ENTRAR E ATIVAR STORAGE</button></div> : <><label className="storage-dropzone"><UploadCloud size={21} /><b>ENVIAR ARQUIVO</b><small>Imagens, PDF, JSON e texto · até 10 MB</small><input type="file" accept="image/*,.pdf,.json,.txt,.md" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} /></label>{message && <p className="storage-message">{message}</p>}<div className="storage-list">{filesQuery.isLoading ? <p className="storage-empty">Consultando arquivos sincronizados…</p> : filesQuery.data?.length ? filesQuery.data.map((file) => <a className="storage-file" href={file.url} target="_blank" rel="noreferrer" key={file.fileKey}>{file.mimeType.startsWith("image/") ? <ImageIcon size={16} /> : <FileText size={16} />}<span><b>{file.fileName}</b><small>{formatFileSize(file.size)} · {new Date(file.createdAt).toLocaleDateString()}</small></span><ExternalLink size={13} /></a>) : <p className="storage-empty">Nenhum arquivo no cofre. Envie seu primeiro briefing.</p>}</div></>}</section></div>;
}
