...
function ArenaView({ state, setState, profile, setProfile, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; profile: CompetitiveProfile; setProfile: React.Dispatch<React.SetStateAction<CompetitiveProfile>>; onBack: () => void }) {
  ...
  return <main className="game-shell realtime-screen"><div className="scanline" /><header className="topbar"><div className="brand-lockup"><div className="brand-mark"><Shield size={16} /></div><div><p className="eyebrow">{state.loadout.patch} {state.loadout.teamName}</p><h1>TACTICAL ARENA / LIVE</h1></div></div><div className="match-meta"><AudioToggle /><button type="button" className="hq-back" onClick={() => { tacticalAudio.ui(); onBack(); }}>QG</button><span className="rival-chip">{rival.operatorName} / {rival.teamName}</span><span className="rank-chip">{rank.rank.insignia} {rank.rank.label}</span></div></header><section className="status-strip"><div className="status-title"><span className="status-dot" /> LIVE CQB</div><div className="round-label">TEMPO REAL <span>•</span> COLISÃO ATIVA</div></section><section className="realtime-panel"><div className="arena-heading"><div><p className="eyebrow">{state.loadout.operatorName} / {state.loadout.teamName}</p><h2>WAREHOUSE <span>TOP-DOWN</span></h2></div><div className="comms"><Radio size={15} /><span>LIVE LINK</span></div></div><RealtimeArenaCanvas key={`${state.enemyLoadout.operatorName}-${report ? "ended" : "live"}`} loadout={state.loadout} rival={rival} onFinish={handleFinish} onExit={onBack} /><div className="realtime-legend"><span><i className="legend-swatch player-swatch" /> {state.loadout.operatorName} / {state.loadout.weapon.toUpperCase()}</span><span><i className="legend-swatch enemy-swatch" /> {rival.operatorName} / {rival.weapon.toUpperCase()}</span></div></section><section className="realtime-command"><div><p className="eyebrow">CONTROLE MOBILE</p><h3>JOYSTICK TÁTICO</h3><small>Movimento analógico 360° com colisão estrita contra paredes.</small></div><div className="realtime-badges"><b>SEM PA</b><b>360°</b><b>COLISÃO ON</b></div></section><footer className="footer-hint"><span><AlertTriangle size={12} /> PAREDES BLOQUEIAM MOVIMENTO</span><span>TOQUE NO MAPA PARA DISPARAR</span></footer>{report && <AfterActionReport report={report} onReplay={restart} onHQ={() => { tacticalAudio.ui(); onBack(); }} />}</main>;
}
