...
export default function RealtimeArenaCanvas({ loadout, rival, onFinish, onExit }: Props) {
  ...
  const visionPath = (s: Runtime) => { const path = new Path2D(); const origin = s.player; const radius = VISION_RADIUS; const spread = 0.58; const rays = 56; path.moveTo(origin.x, origin.y); for (let i = 0; i <= rays; i += 1) { const angle = Math.atan2(s.aim.y, s.aim.x) - spread + (spread * 2 * i / rays); const direction = { x: Math.cos(angle), y: Math.sin(angle) }; let length = radius; for (let step = 10; step <= radius; step += 10) { const point = { x: origin.x + direction.x * step, y: origin.y + direction.y * step }; if (blocked(point, 2)) { length = step; break; } } path.lineTo(origin.x + direction.x * length, origin.y + direction.y * length); } path.closePath(); path.moveTo(origin.x + NEAR_VISION_RADIUS, origin.y); path.arc(origin.x, origin.y, NEAR_VISION_RADIUS, 0, Math.PI * 2); return path; };
  const visibleToPlayer = (s: Runtime, position: Vec) => distance(s.player, position) <= NEAR_VISION_RADIUS || (distance(s.player, position) <= VISION_RADIUS && Math.abs(Math.atan2(position.y - s.player.y, position.x - s.player.x) - Math.atan2(s.aim.y, s.aim.x)) < .6 && hasLineOfSight(s.player, position));
