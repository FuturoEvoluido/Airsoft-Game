...
export default function RealtimeArenaCanvas({ loadout, rival, onFinish, onExit }: Props) {
  ...
  useEffect(() => { const canvas = canvasRef.current; if (!canvas) return; const dpr = window.devicePixelRatio || 1; canvas.width = WORLD.w * dpr; canvas.height = WORLD.h * dpr; const context = canvas.getContext("2d"); if (!context) return; const frame = { current: 0 }; let last = performance.now(); const loop = (now: number) => { const dt = Math.min(.05, (now - last) / 1000); last = now; const s = runtime.current; if (s.running) {
      const speed = loadout.weapon === "smg" ? 190 : loadout.weapon === "sniper" ? 130 : 160; s.player = moveWithCollision(s.player, { x: s.joystick.x * speed * dt, y: s.joystick.y * speed * dt });
    ...
    } draw(context, dpr); frame.current = requestAnimationFrame(loop); }; frame.current = requestAnimationFrame(loop); return () => cancelAnimationFrame(frame.current); }, [loadout.weapon, rival.weapon]);
